...
6. "bullets": 3-4 key findings from the PUBMED ABSTRACTS ONLY — never from the Reddit comments. Each bullet must be a real finding an abstract actually reported, with an accurate "index" pointing at which abstract (1-based) it came from. Never summarize, paraphrase, or reference Reddit/community opinion here — that belongs only in "communityVerdict". If fewer than 3 abstracts have a genuinely relevant finding, return fewer bullets rather than inventing one or borrowing from Reddit content. Each bullet: 1 sentence, 50-150 chars, specific with numbers/stats when available.
